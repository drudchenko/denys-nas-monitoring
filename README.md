...
* **Keyless Authentication:** This project uses **Workload Identity Federation (WIF)**. No long-lived GCP Service Account keys (JSON) are stored in GitHub Secrets. Instead, GitHub Actions authenticates via temporary OIDC tokens.
...
## 🚀 Deployment
1.  **Infrastructure:** Changes to `.tf` files in the `main` branch trigger a GitHub Action that authenticates via WIF and applies changes to GCP.
